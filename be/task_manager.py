        ...
        for file_name in os.listdir(self.tasks_dir):
            if file_name.endswith('.json'):
                with open(os.path.join(self.tasks_dir, file_name), 'r') as file:
                    task = json.load(file)
                    task['file_name'] = file_name  # Add the file name to the task data
                    tasks.append(task)
        return tasks

    def get_tasks(self) -> List[Dict[str, str]]:
        self.tasks = self.load_tasks()  # Reload tasks from disk
        return [{"task": task["task"], "file_name": task["file_name"]} for task in self.tasks]
