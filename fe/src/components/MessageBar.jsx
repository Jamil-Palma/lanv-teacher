...
import { Button, TextField, Typography, AppBar } from "@mui/material";
    ...
    const selectTask = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:8000/task_by_filename/${selectedTaskFilename}`
        );
        const taskDetails = response.data;
        setCurrentTask(taskDetails);

        const startResponse = await axios.post(
          "http://localhost:8000/initialize_task",
          {
            input_text: "",
